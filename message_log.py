...
class MessageLog:
    ...
    def add_message(
        ...
        if 'stack' is true then message can stack with a previous message
        of the same text."""
    ...
    def render(
        ...
        """Render this log over this given area."""
        self.render_messages(console, x, y, width, height, self.messages)

    @staticmethod
    def wrap(string:str, width:int) -> Iterable[str]:
        """Return a wrapped message."""
        for line in string.splitlines(): #split string between newlines
    ...
    def render_messages(
        ...
        for message in reversed(messages):
            for line in reversed(list(cls.wrap(message.full_text, width))):
                ...
                y_offset -= 1
                if y_offset < 0:
                    return #No more space to display messages.