...
If an event handler is returned then it will become the active event handler for future events.
If an action is returned it will be attempted and if it's valid then MainGainEventHandler
will become the active handler.'''

class BaseEventHandler(tcod.event.EventDispatch[ActionOrHandler]):
    def handle_events(self, event:tcod.context.Context) -> BaseEventHandler:
        """Handle the event and return the next active event handler."""
...
class AskUserEventHandler(EventHandler):
    ...
    def ev_keydown(self, event:tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        """By default any key exits this event handler."""
        if tcod.event.Modifier(4098) & event.sym in { #Ignore mod keys
    ...
    def ev_mousebuttondown(self, event: tcod.event.MouseButtonDown) -> Optional[ActionOrHandler]:
        """By default any mouse click exits this event handler."""
        return self.on_exit()   
...
class InventoryEventHandler(AskUserEventHandler):
    ...
    def on_render(self, console: tcod.console.Console) -> None:
        ...
        Will move to a different location based on where the player is, so the player can see where they are."""
        super().on_render(console)
...
class SelectIndexHandler(AskUserEventHandler):
    ...
    def on_render(self, console:tcod.console.Console) -> None:
        ...
        console.rgb["bg"][int(x), int(y)] = color.white
        console.rgb["fg"][int(x), int(y)] = color.black
    ...
    def on_index_selected(self, x, y) -> Optional[ActionOrHandler]:
        """Called when an index is selected."""
        raise NotImplementedError()

class LookHandler(SelectIndexHandler):
    """Lets the player look down using the keyboard."""
    def on_index_selected(self, x, y) -> MainGameEventHandler:
        """Return to the main event handler."""
        return MainGameEventHandler(self.engine)

class SingleRangedAttackHandler(SelectIndexHandler):
    """Handles selecting a single enemy. Only the enemy selected will be affected."""
    def __init__(
    ...
    def on_index_selected(self, x, y) -> Action | None:
        return self.callback((x,y))
    
class  AreaRangedAttackHandler(SelectIndexHandler):
    """Handles selecting an area within a given radius. Any entity within the area will be affected."""
    def __init__(
...
class HistoryViewer(EventHandler):
    """Prints the history in a larger window which can be navigated."""
    ...
    def on_render(self, console:tcod.console.Console) -> None:
        ...
        log_console.print_box(
            0,0, log_console.width, 1, "-|Message History|-", alignment=30
        )
